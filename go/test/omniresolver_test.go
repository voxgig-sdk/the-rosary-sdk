...
import (
	...
	omni "github.com/voxgig-sdk/the-rosary-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TheRosaryError); is {
				out := map[string]any{
					"name":    "TheRosaryError",
...
func omniCtx(arg any, client *sdk.TheRosarySDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TheRosarySDK); is {
			client = live
