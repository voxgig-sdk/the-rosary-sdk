...
//     (\`new TheRosarySDK(...)\` / \`TheRosarySDK.test(...)\` become
//     \`TheRosarySDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"today":{"test01":{"id":"test01"}},"v1n":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new TheRosarySDK(...)\` and any
// \`TheRosarySDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TheRosarySDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TheRosarySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
