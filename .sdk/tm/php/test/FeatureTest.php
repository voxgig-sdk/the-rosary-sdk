...
class FtCtrl extends TheRosaryControl
{
...
class FtClient
{
    ...
    private TheRosaryUtility $utility;

    public function __construct(TheRosaryUtility $utility, array $options)
    {
    ...
    public function get_utility(): TheRosaryUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TheRosaryContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public TheRosaryContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TheRosaryContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TheRosaryConfig::make_feature($name);
    ...
    private static function buildUrl(TheRosarySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TheRosaryError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TheRosaryResponse($fetched);
        }
    ...
    private function fail(TheRosaryContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TheRosaryContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
