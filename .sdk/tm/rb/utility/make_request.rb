...
module TheRosaryUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = TheRosaryResponse.new({})
    ...
    if fetch_err
      ...
      response = TheRosaryResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TheRosaryResponse.new(fetched)
    else
