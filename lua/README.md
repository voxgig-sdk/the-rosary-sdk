...
The Lua SDK for the TheRosary API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:GetRosaryByDay(nil):list(nil, nil)
...
local result, err = client:TheRosary(nil):load(
  { id = "test01" }, nil
)
