...
class TodayEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = TheRosarySDK.test(seed, nil)
    seen = base.Today(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Today(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set THE_ROSARY_TEST_TODAY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def today_basic_setup(extra)
  ...
  entid_env_raw = ENV["THE_ROSARY_TEST_TODAY_ENTID"]
