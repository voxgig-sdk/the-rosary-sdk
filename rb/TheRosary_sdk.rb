...
require_relative 'TheRosary_types'
...
class TheRosarySDK
  ...
  def initialize(options = {})
    ...
    feature_opts = TheRosaryHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = TheRosaryHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, TheRosaryFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    TheRosaryUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = TheRosaryHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = TheRosaryHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = TheRosaryHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => TheRosaryError.new(
        "#{op}_allow",
        "TheRosarySDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = TheRosaryHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = TheRosaryError.new(
        "graphql_error", "TheRosarySDK: graphql: #{msg}")
      res["graphql"] = errors
    end
