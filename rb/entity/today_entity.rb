...
class TodayEntity
  ...
  def data_set(args)
    if args
      @_data = TheRosaryHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = TheRosaryHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [TodayListMatch, Hash, nil] match filter (any subset of
  #   Today fields); defaults to nil, treated as an empty match that lists all.
