...
boru-driven command-line client **and** interactive REPL for the TheRosary
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/the-rosary-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/the-rosary-cli list today
   ```
...
Run with no arguments to open a REPL (prompt `the-rosary>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `today`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/the-rosary-cli`. |
...
(`./the-rosary-cli <expr>`) and interactively (the REPL), and expressions compose the
