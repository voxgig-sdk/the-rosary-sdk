...
[MCP](https://modelcontextprotocol.io) server exposing the TheRosary SDK as
two agent tools — `the-rosary_list` and `the-rosary_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/the-rosary-mcp)
make build
...
```jsonc
// the-rosary_list: first page of records
{ "entity": "today" }
{ "entity": "today", "query": { } }

// the-rosary_load: one record by id
{ "entity": "v1n", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/the-rosary-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user the-rosary \
     -- "$PWD"/dist/*/the-rosary-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `the-rosary_list` and `the-rosary_load` tools now appear
   in new sessions. Ask the agent to *"list today using the-rosary"*
   and it calls `the-rosary_list` with `{"entity":"today"}`.
...
```sh
export THE_ROSARY_APIKEY=sk_live_xxx            # API key
export THE_ROSARY_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `the-rosary_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `the-rosary_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `THE_ROSARY_APIKEY` | API key sent with every request. |
| `THE_ROSARY_BASE` | Optional override of the API base URL. |
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"the-rosary_load","arguments":{"entity":"v1n","query":{"id":1}}}}'
```
