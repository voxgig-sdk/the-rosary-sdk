...
class V1nEntity
{
    ...
    /**
     * @param V1n|array $args V1n data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = TheRosaryHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return V1n|array The current V1n data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of V1n fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = TheRosaryHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of V1n fields).
     */
     ...
     * Load a single V1n.
     *
     * @param V1nLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed V1nLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return V1n|array The loaded V1n as an assoc-array at the
     *   SDK boundary; throws TheRosaryError on failure (item-5 convention).
     */
