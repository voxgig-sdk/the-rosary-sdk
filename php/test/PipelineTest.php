...
{
    private static function utility(): TheRosaryUtility
    {
        return new TheRosaryUtility();
    }
    ...
    private static function ctx(array $over = []): TheRosaryContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new TheRosaryContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new TheRosaryOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof TheRosaryError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new TheRosaryOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = TheRosaryMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new TheRosaryOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = TheRosaryMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new TheRosaryOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = TheRosaryMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TheRosarySpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = TheRosaryMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new TheRosaryResponse([]),
            'result' => new TheRosaryResult([]),
        ]);
        [, $err] = TheRosaryMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new TheRosarySpec([]),
            'response' => null,
            'result' => new TheRosaryResult([]),
        ]);
        [, $err] = TheRosaryMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new TheRosarySpec(['step' => 's']),
            'response' => new TheRosaryResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new TheRosaryResult([]),
        ]);
        [, $err] = TheRosaryMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new TheRosarySpec(['step' => 's']),
            'response' => new TheRosaryResponse(self::resp(200, ['v' => 1])),
            'result' => new TheRosaryResult([]),
        ]);
        [, $err] = TheRosaryMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new TheRosarySpec(['step' => 's']),
            'response' => new TheRosaryResponse(self::resp(200, ['v' => 2])),
            'result' => new TheRosaryResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        TheRosaryMakeResponse::call($ctx);
    ...
    {
        $preset = new TheRosaryResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new TheRosarySpec([]),
            'response' => new TheRosaryResponse([]),
            'result' => new TheRosaryResult([]),
        ...
        [$got, $err] = TheRosaryMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new TheRosaryResult([])]);
        [, $err] = TheRosaryMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new TheRosarySpec([]), 'result' => null]);
        [, $err] = TheRosaryMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TheRosarySpec(['step' => 's']),
            'result' => new TheRosaryResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new TheRosaryOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TheRosaryMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TheRosarySpec(['step' => 's']),
            'result' => new TheRosaryResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new TheRosaryOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TheRosaryMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TheRosaryResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new TheRosarySpec([]),
            'result' => new TheRosaryResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = TheRosaryMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TheRosaryMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new TheRosaryError('boom', 'boom');
        $utility->fetcher = function (TheRosaryContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TheRosarySpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = TheRosaryMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (TheRosaryContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = TheRosaryMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(TheRosaryResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (TheRosaryContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TheRosarySpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = TheRosaryMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new TheRosaryResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new TheRosarySpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = TheRosaryMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = TheRosaryMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = TheRosaryMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new TheRosaryResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, TheRosaryDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new TheRosaryResult(['ok' => false])]);
        $this->expectException(TheRosaryError::class);
        TheRosaryDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new TheRosaryResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', TheRosaryMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        TheRosaryMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new TheRosaryBaseFeature();
        $b = new TheRosaryBaseFeature();
        TheRosaryFeatureAdd::call($ctx, $a);
        TheRosaryFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): TheRosaryBaseFeature
    {
        $f = new TheRosaryBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        TheRosaryFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): TheRosaryContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new TheRosarySpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TheRosaryPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new TheRosaryResponse(['headers' => null]),
            'result' => new TheRosaryResult([]),
        ]);
        TheRosaryResultHeaders::call($ctx);
