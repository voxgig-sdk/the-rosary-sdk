...
class TheRosaryRetryFeature extends TheRosaryBaseFeature
{
    ...
    public function init(TheRosaryContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (TheRosaryContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(TheRosaryContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(TheRosaryContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
