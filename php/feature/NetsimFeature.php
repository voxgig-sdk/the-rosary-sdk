...
class TheRosaryNetsimFeature extends TheRosaryBaseFeature
{
    ...
    private function _simulate(TheRosaryContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(TheRosaryContext $ctx, array $applied): void
    {
        $client = $this->client;
