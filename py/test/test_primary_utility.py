...
from therosary_sdk.utility.voxgig_struct import voxgig_struct as vs
from therosary_sdk import TheRosarySDK
from therosary_sdk.core.spec import TheRosarySpec
from therosary_sdk.core.result import TheRosaryResult
from therosary_sdk.core.response import TheRosaryResponse
from therosary_sdk.core.operation import TheRosaryOperation
from therosary_sdk.core.error import TheRosaryError
from therosary_sdk.core import helpers
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return TheRosaryError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = TheRosarySDK.test(None, None)
        ...
        class TestInitFeature(TheRosaryBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(TheRosaryBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = TheRosarySpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = TheRosarySDK.test(None, setup_opts)
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = TheRosarySDK.test(None, setup_opts)
